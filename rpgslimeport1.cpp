...
"em 2 situações diferentes: as telas de batalha e as telas de descanso. Nas telas de batalha você receber�á "
...
"Aproveite o jogo!";
	...
	cout << "Espero que o jogo satisfa�a a Igreja" << "\n";
	}
	else if(nome == "Arthur")
		cout << "Colega ne�fito, voc� recebeu o jogo? Espero que goste" << "\n";
	else if(nome == "Tobias")
		cout << "Espero que o jogo agrade o grande Blob" << "\n";
	else if(nome == "Stalin")
		cout << "Sauda��es, stalin, espero que aproveite o jogo.\nEu poderia ter escrito alguns blogs para a Arc�dia ao inv�s de fazer isso..." << "\n";
	else if(nome == "Renan")
		cout << "Prossiga, s�mio"<< "\n";
	else if(nome == "Dias")
		cout << "J� leu Douluo Dalu?"<< "\n";
	...
	sleep(1);
	cout << nome << " caminha calmamente pelas plan�cies procurando por uma slime" << "\n";
	sleep(1);
	cout << "." << "\n";
	sleep(1);
	cout << "." << "\n";
	...
	cout << "." << "\n";
	sleep(1);
	cout << "Voc� encontrou uma slime rosa � sua primeira slime, ent�o voc� n�o ter� a op��o de fugir";
	cout << "\n";
	sleep(2);
	...
	while (1){
		...
		while (1){
			...
			if (acao == 1)
				...
				cout << "ERRO! Insira um n�mero v�lido" << "\n";
			sleep(1);
	...
	while (1){
		...
		while (1){
			...
			if (sorteio == 1){
				cout << nome << " descansa no gramado ap�s a batalha" << "\n";
			}
			else if (sorteio == 2){
				...
				cout << nome << " trope�ou num pedalo de ouro enquanto voltava para casa!" << "\n";
				dinheiro += 4;
			}
			
			sorteio = randint(2, 3);
			vidapj += sorteio;
			cout << "Ap�s descansar " << nome << " recuperou " << sorteio << " de vida!" << "\n";
			sleep(2);
			...
			while (1){
				...
				if (acao == 1){
					...
					while (1){
						titulo("Menu");
						cout << "[1] Status || [2] Besti�rio || [3] Sair do Menu" << "\n";
						
						int acao2;
						cin >> acao2;
						cout << "Aperte 1 para ver os status, 2 para abrir o Besti�rio ou 3 para sair " << "\n";
						if (acao2 == 1){
							...
							titulo("Besti�rio");
							if (mortet >= 1){
								cout << "Slimes s�o seres gelatinosos e territoriais que habitam nosso mundo. Caso voc� enfure�a uma slime\ntome cuidado, especialmente se for uma slime azul!" << endl;
							}
							else
								cout << "??????????????" << endl;
							if (mortet >= 3){
								cout << "Segundo nossos especialistas, quanto mais vida a slime tem mais forte ela �, ent�o slimes que estiverem\ncom vida alta s�o especialmente perigosas!" << endl;
							}
							else
								cout << "??????????????" << endl;
							int acao3;
		...
		sorteio = randint(0, 2);
		if (sorteio == 0)
			cout << nome << " caminha calmamente pelas plan�cies procurando uma slime" << "\n";
		else if(sorteio == 1)
			cout << nome << " ca�a vorazmente por mais uma slime" << "\n";	
		else
		...
		while (1){
			...
			while (1){				
				...
				if (battle_action == 1) { // Tatakae
					...
					if (sorteio !=0){
						fuga = true;
						cout << "Voc� tentar� fugir ap�s o turno da slime" << "\n";
						break;
					}
				}
				else {	
			...
			if (battle_action == 2 && fuga){
				...
				cout << nome << " n�o conseguiu fugir e foi acertado no caminho!" << "\n" << "-1 de vida!" << "\n";
				vidapj -= 1;
				if (vidapj == 0){
					vivo = false;
					sleep(1);
		...
		if (!fuga && inimigo.vida == 0){
			...
			cout << "Voc� voltar� para a tela de descanso em breve." << "\n";
			fuga = false;
			sleep(2);	
		}
	}
	...
	if (!vivo){
		cout << "Voc� morreu, tente novamente!" << "\n";
		sleep(1);
	}
	else{
		cout << "Voc� conseguiu 50 de dinheiro e completou o objetivo do jogo, parab�ns!" << endl;
	}
